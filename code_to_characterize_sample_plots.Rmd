...
```{r}
raw_data <- read_csv("CopyOfdata.csv") %>%
  select(
...
politically moderate (___%) or liberal (__%), 
...
g <- raw_data %>%
  ...
  ylab("Number of Participants") +
  scale_y_continuous(expand = c(0,0)) +
  annotate("text", x = 4, y = 1550, label = "Note: 4/4860 participants \n had unknown ages", size = 2.5) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
  theme(panel.background = element_blank(), axis.ticks = element_blank(), 
            axis.line = element_line(color = "black"),
    axis.title.x = element_text(vjust = 1))
...
4 participants had "unknown" or "refused 



college degree or higher (___%)."
...
g <- raw_data %>%
  ...
  ylab("Number of Participants") +
  scale_y_continuous(expand = c(0,0)) +
  annotate("text", x = 2, y = 2000, label = "Note: 4/4860 participants \n had unknown education", size = 2.5) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
  theme(panel.background = element_blank(), axis.ticks = element_blank(), 
            axis.line = element_line(color = "black"),
    axis.title.x = element_text(vjust = 1))
...
g <- raw_data %>%
  ...
  group_by(MARITAL_W56) %>%
  mutate(
    n_total_for_var_1 = sum(n)
  ) %>%
  group_by(F_USR_SELFID) %>%
  mutate(
    n_total_for_var_2 = sum(n)
  ) %>%
  mutate(
    n_as_pct_of_var1 = 100*round(n/n_total_for_var_1, digits = 2)
  ) %>%
  mutate(
    n_as_pct_of_var2 = 100*(round(n/n_total_for_var_2, digits = 2))
  ) %>%
  select(-n_total_for_var_1, -n_total_for_var_2) %>%
      ggplot(aes(
        x = MARITAL_W56,
        y = F_USR_SELFID,
        fill = n,
        text = paste(paste(paste("n:", n, sep = " "),
                     paste("N as a percentage of people with %{x}:", paste(n_as_pct_of_var1, "%", sep = "")),
                     sep = "<br>"),
                     paste("N as a percentage of people with %{y}:", paste(n_as_pct_of_var2, "%", sep = "")), sep = "<br>")
      )) +
      geom_tile() +
...
```{r}
raw_data$FAMSURV19DATING_W56 <- factor(raw_data$FAMSURV19DATING_W56,levels = c("Refused", "Not at all well", "Not too well", "Fairly well", "Very well"))
...
```{r}
    raw_data$DATE10YR_W56 <- factor(raw_data$DATE10YR_W56,levels = c("Refused", "Harder today", "About the same", "Easier today"))
...
g <- raw_data %>%
      ...
      xlab("In general in the past year, has using online dating sites or dating apps made you feel more hopeful or frustrated?")+
      ylab("Sex")+
      scale_x_discrete(labels = function(x) str_wrap(x, width = 10))+
      scale_fill_gradient(low = "#FFFFFF", high = "#000773", na.value = "#8E8E8E") 
...
```{r}
## if no people got asked both questions, then the data frame will be totally empty. We only want to generate the plot if data is not empty.

data <- raw_data %>%
        filter(!is.na(PARTNERSEX_W56)) %>%
        filter(!is.na(SEEKING_W56)) %>%
        ### group_by_ from https://stackoverflow.com/questions/54482025/call-input-in-shiny-for-a-group-by-function
        group_by(PARTNERSEX_W56,
         SEEKING_W56) %>%
        summarize(
          n = n()
        )

## if dim(data)[1] == 0 {renderText("you can't do that stop")}
## else{renderPlotly({})}
...
total_asked <- raw_data %>%
      ...
      summarize(
        ...
        num_refused = sum(ONFEEL.c_W56 == "Refused"),
