...
total_asked <- raw_data %>%
  select(
        lookup_questions %>%
          filter(questions == "How important is it to you that online profiles included religious beliefs?") %>%
          pull(var_names[1])) %>%
  summarize(
        n = n(),
...
g <- ggplot(raw_data %>%
      ...
      geom_text(aes(x = length(levels(get(lookup_questions %>%
                  filter(questions == "How important is it to you that online profiles included religious beliefs?") %>%
                  pull(var_names[1])))) + 2, y = (1.1*max(n)), label = paste(paste(total_asked, total_people, sep = "/"), "total participants were asked this question", sep = " ")),
                 hjust = 1, vjust = -1)
