...
for(i in 1:ncol(raw_data)) {
  ...
  for(j in 1:length(scales)){
    ...
    if (
      identical(
        sort(unlist(unique(raw_data[i]))), 
        sort(unlist(scales[j]))
      )
    ){ 
      
      print(levels(raw_data[i]))
      
      raw_data[i] <- factor(raw_data[i], levels = scales[j])
      
      print(levels(raw_data[i]))
      
    }
  #  else{
  #    print("column i does not match scale")
  #  }
  }
}
